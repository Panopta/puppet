...
define panopta::modules (
  ...
  if (($::panopta_vesion != undef) and ($::panopta_customer_key != undef) and ($::panopta_server_key != undef) and ($::panopta_agg_url != undef) or
     (($::panopta_vesion != '') or ($::panopta_customer_key != '') or ($::panopta_server_key != '') or ($::panopta_agg_url != ''))) {
    # Building the default configuration for Panopta so we can add modules
    # Only if the base config has been created after the initial installation.
    # Since the facts are not here yet
    notify{'test': }
